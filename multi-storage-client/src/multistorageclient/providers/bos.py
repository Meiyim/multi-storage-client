...
# Backend selection: "auto" prefers the Go client (bos_tool) and falls back to
# bosfs; "go" forces the Go client; "bosfs" forces the fsspec client.
_BACKEND_AUTO = "auto"
_BACKEND_GO = "go"
...
class BaiduBosStorageProvider(BaseStorageProvider):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        access_key = access_key or os.getenv("BCE_ACCESS_KEY_ID")
        secret_key = secret_key or os.getenv("BCE_SECRET_ACCESS_KEY")

        self._backend = self._create_backend((backend or _BACKEND_AUTO).lower(), access_key, secret_key, token, kwargs)
        logger.debug("BOS provider using '%s' backend (endpoint=%s)", self._backend.name, self._endpoint_url)

    def _create_backend(self, backend: str, access_key, secret_key, token, bosfs_kwargs: dict):
        if backend in (_BACKEND_AUTO, _BACKEND_GO):
            try:
                return _BosGoBackend(access_key, secret_key, token, self._bns, self._endpoint_url, self._go_lib_path)
            except Exception as error:  # noqa: BLE001 — fall back unless the Go backend was explicitly required
                if backend == _BACKEND_GO:
                    raise
                logger.warning("BOS Go backend unavailable (%s); falling back to bosfs.", error)
        return _BosfsBackend(access_key, secret_key, token, self._endpoint_url, **bosfs_kwargs)

    @property
    def backend_name(self) -> str:
        """Which backend is active: ``"go"`` or ``"bosfs"``."""
    ...
    def _is_dir(self, path: str) -> bool:
        ...
        objects, _, _ = self._backend.list_page(bucket, prefix, "/", "", 1)
        return len(objects) > 0

    def _get_object(self, path: str, byte_range: Optional[Range] = None) -> bytes:
        bucket, key = split_path(path)
        if byte_range is not None:
            return self._backend.get_range(bucket, key, byte_range.offset, byte_range.size)
        return self._backend.get_bytes(bucket, key)
    ...
    def _put_object(
        ...
        bucket, key = split_path(path)
        self._backend.put_bytes(bucket, key, body)
        return len(body)
    ...
    def _copy_object(self, src_path: str, dest_path: str) -> int:
        ...
        self._backend.copy(dst_bucket, dst_key, src_bucket, src_key)
    ...
    def _delete_object(self, path: str, if_match: Optional[str] = None) -> None:
        ...
        bucket, key = split_path(path)
        self._backend.delete(bucket, key)
    ...
    def _delete_objects(self, paths: list[str]) -> None:
        ...
        for bucket, keys in by_bucket.items():
            self._backend.delete_many(bucket, keys)
    ...
    def _get_object_metadata(self, path: str, strict: bool = True) -> ObjectMetadata:
        bucket, key = split_path(path)
        try:
            meta = self._backend.head(bucket, key)
        except FileNotFoundError:
    ...
    def _list_objects(
        ...
        while True:
            objects, is_truncated, next_marker = self._backend.list_page(bucket, prefix, delimiter, marker, 1000)
            for obj in objects:
                rel_key = str(obj["key"])
    ...
    def _upload_file(self, remote_path: str, f: Union[str, IO], attributes: Optional[dict[str, str]] = None) -> int:
        ...
        if isinstance(f, str):
            self._backend.upload_file(bucket, key, f)
    ...
    def _download_file(self, remote_path: str, f: Union[str, IO], metadata: Optional[ObjectMetadata] = None) -> int:
        ...
        if isinstance(f, str):
            ...
            self._backend.download_file(bucket, key, f)
            return os.path.getsize(f)

        data = self._backend.get_bytes(bucket, key)
